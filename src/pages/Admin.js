...
const Admin = () => {
  ...
  const handleSearch=()=>{
    return regUsers?.filter((user)=>
      ...
      user.Weight.toLowerCase().includes(search.toLowerCase())
    )
  }
  const delUser=(user)=>{
    if(window.confirm("Are you sure you want to delete"))
    {
      ...
      curr.name!==user.name || curr.email!==user.email || curr.phone!==user.phone || curr.weight!==user.weight || curr.age!==user.age
      )
      setRegUsers(data);
    }
  }
  return (
    <div className={classes.cont}>
      <h1 style={{color:'white',fontSize:'40px',marginTop:'70px',marginBottom:'50px'}}>Registered Users</h1>
      {!regUsers || regUsers.length===0?<h1 style={{color:'white',fontSize:'40px'}} className='stroke'>No User Registered Yet</h1>:
      <div style={{
        width:'fit-content',
        display:'flex',
        justifyContent:'center',
      ...
      <TableContainer className={classes.table}>
        ...
        <TableHead style={{
          ...
          <TableRow>
            {['Name','Email','Phone Number','Age','Weight',''].map((head)=>{
              return (
      ...
      :
      <>
      <h1 style={{color:'white'}}>No Registration Found with this input keywords</h1>
      </>}
      </div>
      }
    </div>
